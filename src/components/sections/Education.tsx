              ...
              return (
                ...
                >
                  ...
                  <div className="hidden md:flex absolute left-1/2 transform -translate-x-1/2 -translate-y-1 z-10">
                    <div className="w-4 h-4 bg-gradient-to-r from-blue-500 to-purple-600 rounded-full ring-4 ring-gray-900 shadow-lg"></div>
                  </div>

                  {/* Content Card */}
                  <div className={`flex-1 ${isEven ? 'md:text-right md:pr-12' : 'md:pl-12'}`}>
                    <div className="bg-gray-800 rounded-2xl p-6 md:p-8 shadow-lg hover:shadow-xl transition-shadow border border-gray-700">
                      {/* Header */}
                      ...
                      {/* Date & Location */}
                      <div className={`flex flex-wrap items-center gap-4 mb-4 text-sm text-gray-400 ${isEven ? 'md:justify-end' : ''}`}>
                        <div className="flex items-center gap-2">
                          <Calendar className="w-4 h-4" />
                          <span>
                            {formatDate(edu.startDate)} -{' '}
                            {edu.current ? (
                              <span className="text-green-400 font-medium">Devam Ediyor</span>
                            ) : (
                      ...
                      {edu.description && (
                        <div 
                          className={`prose prose-sm max-w-none text-gray-300 mb-4 prose-invert ${isEven ? 'md:prose-right' : ''}`}
